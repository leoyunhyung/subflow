...
SubFlow는 SaaS 벤더가 구독 플랜을 등록하고, 유저가 구독·결제하며, 플랫폼이 벤더별 수수료를 자동 정산하는 전체 플로우를 구현한 백엔드 API입니다. 또한 Claude API를 통합하여 구독 이탈 예측 기능을 LLM Gateway 패턴으로 구현했습니다.
...
- **Claude API 기반 구독 이탈 예측** — LLM Gateway 패턴 추상화, 위험군 사전 필터링, JSON 스키마 검증, Celery Beat 일일 배치
...
│   ├── churn_prediction/ # Claude API 기반 구독 이탈 예측 (LLM Gateway 패턴)
│   └── common/           # 공통 권한 클래스
├── config/
...
## 구독 이탈 예측 (Churn Prediction)

Claude API 기반 LLM을 활용한 구독 이탈 예측 시스템입니다.

### 설계 원칙

- **하이브리드 구조** — 룰 기반 사전 필터링 → LLM 정성 분석
- **Gateway 패턴** — `BaseLLMGateway` 추상화로 LLM 제공자 교체 가능 (Claude → 다른 LLM)
- **비용 통제** — 위험군 후보만 LLM 호출 + 결과 캐싱
- **응답 안정성** — JSON 스키마 검증으로 LLM 응답 불안정성 대응
...
┌──────────────────────────────────────────────────┐
│  1. 룰 기반 위험군 사전 필터링                    │
│     (결제 실패 횟수, 로그인 빈도 감소 등)         │
└──────────────────┬───────────────────────────────┘
                   ▼
┌──────────────────────────────────────────────────┐
│  2. 피처 추출 (Feature Extractor)                │
│     유저별 구독·결제·활동 데이터 → 피처 스냅샷    │
└──────────────────┬───────────────────────────────┘
                   ▼
┌──────────────────────────────────────────────────┐
│  3. LLM Gateway → Claude API 호출                │
│     이탈 확률 + 사유 자연어 응답                  │
└──────────────────┬───────────────────────────────┘
                   ▼
┌──────────────────────────────────────────────────┐
...
LLM에 보낸 입력 피처와 받은 응답을 함께 저장하여, 동일 입력에 대한 응답의 합리성을 사후 검증할 수 있도록 했습니다. 이는 정산 시스템의 SettlementHistory 패턴을 LLM 영역에 재사용한 구조입니다.
...
**현재 73개 테스트 통과 / GitHub Actions CI 자동 실행**
...
| GET | `/api/v1/churn-prediction/{user_id}/` | 단건 이탈 예측 조회 | Admin |
| POST | `/api/v1/churn-prediction/batch/` | 이탈 예측 배치 실행 | Admin |
